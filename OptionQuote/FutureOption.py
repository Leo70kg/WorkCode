...
class VolCal(OptionCalBase):
    ...
    StartDate = w.tdaysoffset(-m, EndDate, "Period=M").Data[0][0]
    ...
    def __init__(self, code, t):
        ...
        """计算日级别波动率"""
        self.daily_ret = Util.get_daily_ret(self.cursor, code, VolCal.StartDate, VolCal.EndDate)
        self.daily_vol_series = self.daily_ret.rolling(window=self.baseT, center=False).std().dropna() * np.sqrt(252)
    ...
    def base_vol(self):
        ...
        ewma_daily_vol = pd.ewma(self.daily_ret**2, span=32)
        base_vol = np.sqrt(ewma_daily_vol.iloc[-1]) * np.sqrt(252) * 0.7 + \
                   0.3 * self.minute_vol_series[(-22 * self.num):].ewm(span=32).mean().iloc[-1]

        standard_line = (np.percentile(self.daily_vol_series, 50) + np.mean(self.daily_vol_series)) * 0.5
        base_vol = base_vol if base_vol >= standard_line else 0.7*standard_line + base_vol*0.3
